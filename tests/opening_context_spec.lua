...
describe("opening context", function()
	it("provides one valid default context", function()
		...
		assert.are.equal("local_1v1_practice", context.recommendedEnvironment)
		...
		assert.are.equal("Закончи три mex и добавь wind или solar.", context.milestones[1].action)
		assert.are.equal("Три mex и хотя бы один wind или solar закончены.", context.milestones[1].doneWhen)
	...
	it("rejects missing milestone completion text", function()
		local context = OpeningContext.get()
		context.milestones[1].doneWhen = nil

		local valid, reason = OpeningContext.validate(context)
		assert.are.equal(false, valid)
		assert.are.equal("context milestone invalid", reason)
	end)
