...
local function setStatus(name, ok, detail)
	apiStatus[name] = {
		ok = ok == true,
		detail = detail,
	}
...
local function safeCall(name, fn, ...)
	if type(fn) ~= "function" then
		setStatus(name, false, "missing")
		return nil
	end

	local ok, a, b, c, d, e, f, g, h, i = pcall(fn, ...)
	if not ok then
		setStatus(name, false, tostring(a))
		return nil
	end

	setStatus(name, true, "ok")
...
local function detectApi()
	...
	setStatus("GetGameFrame", type(api.getGameFrame) == "function", type(api.getGameFrame) == "function" and "ok" or "missing")
	setStatus("GetGameSeconds", type(api.getGameSeconds) == "function", type(api.getGameSeconds) == "function" and "ok" or "missing")
	setStatus("GetTeamResources", type(api.getTeamResources) == "function", type(api.getTeamResources) == "function" and "ok" or "missing")
	setStatus("GetViewGeometry", type(api.getViewGeometry) == "function", type(api.getViewGeometry) == "function" and "ok" or "missing")
	setStatus("TeamID", type(api.getTeamID) == "function", api.teamIDName)
end

local function readResource(resourceName)
	local current, storage, pull, income, expense, share, sent, received, excess =
		safeCall("GetTeamResources:" .. resourceName, api.getTeamResources, snapshot.teamID, resourceName)
...
local function collectSnapshot()
	...
	if type(seconds) ~= "number" and type(snapshot.gameFrame) == "number" then
		...
		setStatus("GameTimeFallback", true, "frame/30")
	elseif type(seconds) == "number" then
		setStatus("GameTimeFallback", true, "not used")
	else
		setStatus("GameTimeFallback", false, "frame unavailable")
	end
...
local function buildLines()
	...
	append(lines, "API status", "see below")
	...
	for i = 1, #names do
		...
		if status then
			lines[#lines + 1] = "  " .. name .. ": " .. (status.ok and "ok" or "unknown") .. " (" .. tostring(status.detail) .. ")"
		else
			lines[#lines + 1] = "  " .. name .. ": unknown"
		end
	end
...
local function getViewGeometry()
	...
	if type(vsx) ~= "number" or type(vsy) ~= "number" then
		return 1920, 1080
	end
