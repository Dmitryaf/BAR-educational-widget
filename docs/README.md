...
Общие правила AI Rules Hub, специализация BAR и выбранные исключения образуют отдельный нормативный слой. Project-документы объясняют продукт и систему, но не являются agent rules и не переопределяют этот слой. Локальная точка входа для инструментов — `AGENTS.md`.
...
- `tasks/` — активные и завершённые рабочие scopes;
- `docs/archive/` — исторические срезы;
- raw runtime CSV/infolog;
- подробные replay manifests и per-player analyses;
- `AGENTS.md` и применённый `.ai-rules/` layer до отдельного owner decision о публикации правил.

Эти материалы помогают воспроизвести работу, но tasks, archive и runtime evidence не переопределяют current docs или нормативный слой.
